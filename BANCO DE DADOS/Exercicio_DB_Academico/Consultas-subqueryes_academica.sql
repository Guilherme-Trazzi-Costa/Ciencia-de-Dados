...
SELECT 
    E.nome,
    COUNT(M.id_disciplina) AS qtd_disciplinas
...
HAVING COUNT(M.id_disciplina) > (
    SELECT AVG(qtd)
    FROM (
        SELECT COUNT(M2.id_disciplina) AS qtd
        FROM tb_matriculas AS M2
        GROUP BY M2.id_estudante
    ) AS sub
);
-- Exercício 3: Encontre os professores que ministram disciplinas com a maior média de notas.
SELECT P.nome, AVG(nota) FROM tb_professores AS P
INNER JOIN tb_disciplinas AS D
...
SELECT E.nome, AVG(nota) FROM tb_estudantes AS E
INNER JOIN tb_cursos AS C
ON E.id_curso = C.id_curso
INNER JOIN tb_disciplinas AS D
ON C.id_curso = D.id_curso
